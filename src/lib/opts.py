...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--val_intervals', type=int, default=200, #remove the eval
    ...
    self.parser.add_argument('--prefix', type=str, default='',
                             help='specify the annotation files, ['', ‘keep1_’]')
    # for SPLG
    self.parser.add_argument('--sim_thresh', default=0.6, type=float,
                             help="threshold used to create pseudo class label")
    self.parser.add_argument('--use_plg', action='store_true',
                             help='whether use the pseudo class label to update the hm')
    self.parser.add_argument('--sigmoid_thr', default=0.5, type=float,
                             help='specify a threshold for counting the number of position with score(after sigmoid) ' \
                                  'larger than specified threshold')
    self.parser.add_argument('--multi_factor', default=1., type=float,
                             help='a factor for scaling the pseudo category heatmap')
    ## for PGCL
    self.parser.add_argument('--use_gcl', action='store_true',
                             help="whether use the pixel-level group contrastive loss")
    self.parser.add_argument('--gcl_weight', default=0.1, type=float,
                             help="weight of gcl loss")
    self.parser.add_argument('--gcl_tau', default=0.07, type=float,
                             help="temperature factor for pixel-level group contrastive loss")
    self.parser.add_argument('--gcl_topk', default=128, type=int,
                             help="number of positive pos selected for pixel-level group contrastive loss")
    self.parser.add_argument('--gcl_loss_mask', action="store_true",
                              help="do not compute the classified loss on the position selected by gcl")
    # for visualization
    self.parser.add_argument('--visualize_path', type=str,
                             help="path to images for visualization")
    self.parser.add_argument('--demo_save_name', default='result', type=str,
                              help="detect result file name")
    self.parser.add_argument('--ret_copy', action='store_true',
                             help="return a copy image only with geometry transformation")
    self.parser.add_argument('--vis_prefix', default="vis",
                             help="prefix for rename the visualized image")
    self.parser.add_argument('--anno_path', type=str,
                             help="path to annotation file containing information of images to be visualized")
  def parse(self, args=''):
    if args == '':
    ...
    opt.gpus = [i for i in range(len(opt.gpus))] if opt.gpus[0] >=0 else [-1] # will reset to [0,1,2,3]
